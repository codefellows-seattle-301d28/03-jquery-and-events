...
articleView.handleMainNav = function() {
  $('.tab').on('click', function(e) {
    e.preventDefault();
    $('.tab-content').hide();
    let sectionId = $(this).attr('data-content');
    $('#' + sectionId).fadeIn(350);
  });
...
articleView.setTeasers = function() {
  ...
  $('.read-on').on('click', function(e) {
    e.preventDefault();
    let snip = $(this);
    let toggleText = 'Read on &rarr;'
    if (snip.text().indexOf('Read') > -1 ) toggleText = 'Show Less &rarr;';
    snip.html(toggleText);
    $(this).parent().find('*:nth-of-type(n+2)').fadeToggle(350);
  });
