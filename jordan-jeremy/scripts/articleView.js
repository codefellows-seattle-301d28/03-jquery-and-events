...
=======
      $('article').hide();
>>>>>>> 111dc15f828d64abe2fcabc9f69099500f948033
...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    // REVIEWED: Inside this function, "this" is the element that triggered the event handler function we are defining. "$(this)" is using jQuery to select that element (analogous to event.target that we have seen before), so we can chain jQuery methods onto it.
    if ($(this).val()) {
      // DONE: If the <select> menu was changed to an option that has a value, we first need to hide all the articles, and then show just the ones that match for the category that was selected.
      // Use an "attribute selector" to find those articles, and fade them in for the reader.
      $('article').fadeOut();
...
articleView.setTeasers = function() {
  ...
  $('.read-on').on('click', function(e){
    e.preventDefault();
    $(this).hide();
    let $chosenArticle = $(this).prev()
    $chosenArticle.children().show();
  })
